...
/**
 * Processes individual script modules for a specific directory. These are
 * registered as ES modules and enqueued separately via wp_enqueue_script_module.
 *
 * @since  1.0.0
 * @param  {string} dir
 * @return {Object.<string, string>}
 */
const groupScriptModules = (dir) => {
	return globSync(`./resources/js/${dir}/*.js`).reduce(
		(files, filepath) => {
			const name = path.parse(filepath).name;

			files[`js/${dir}/${name}`] = path.resolve(
				process.cwd(),
				`resources/js/${dir}`,
				`${name}.js`
			);
